...
const allAssignedKeys = computed(() => prizes.value.map(prize => prize.keys).reduce((acc, curr) => acc + curr, 0))
const lessKeysInStock = computed(() => allAssignedKeys.value > inventory.value.keys)
</script>
<template>
  <div class="flex gap-4">
    <div>Keys in inventory: <span class="font-medium">{{ inventory.keys }}</span></div>
    <div>Assigned keys: <span class="font-medium" :class="{ 'text-red-500': lessKeysInStock }">{{ allAssignedKeys }}</span></div>
  </div>
  <div v-if="lessKeysInStock" class="bg-yellow-200 flex gap-2 items-center py-1 px-2 rounded-md mt-2">
    <svg xmlns="http://www.w3.org/2000/svg" width="800px" height="800px" viewBox="-0.5 0 25 25" class="fill-yellow-200 size-8">
