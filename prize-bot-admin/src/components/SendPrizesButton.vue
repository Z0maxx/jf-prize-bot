...
async function trySendPrizes() {
  if (hasChanges.value.has('Players')) {
    await playerStore.saveAsync()
  }

  if (hasChanges.value.has('Prizes')) {
...
<template>
  <button
    @click="trySendPrizes"
    class="flex min-w-40 cursor-pointer items-center justify-center gap-2 rounded-md border-2 border-violet-950 bg-violet-900 fill-white text-xl font-medium text-white hover:bg-violet-800"
  >
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      class="size-6 fill-white"
      :class="{ 'animate-bounce': isSendingPrizes }"
    >
      <path
