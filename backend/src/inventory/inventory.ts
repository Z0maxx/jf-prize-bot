...
function isNormalWeapon(item: Item) {
  ...
  return (
    getTagNameForCategoryName(item, categoryNames.quality) === tagNames.unique &&
...
function getUnusualEffectName(item: Item) {
  return item
    .descriptions!.find((desc) => desc.value.startsWith(descriptionValues.unusualEffect))!
    .value!.split(':')[1]!
    .trimStart()
}
...
function getItemName(item: Item) {
  const originalName = item.fraudWarnings
...
async function getSchemaAsync(schema: string) {
  const resp = await fetch(
    `https://raw.githubusercontent.com/danocmx/node-tf2-static-schema/refs/heads/master/static/${schema}.json`,
  )
